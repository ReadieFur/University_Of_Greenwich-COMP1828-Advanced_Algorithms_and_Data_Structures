...
class BellmanFordsAlgorithmDP(AAlgorithm):
    ...
    def find_shortest_path(graph: Graph, start_node: Node, end_node: Node) -> List[PathPart]:
        ...
        for node in graph.nodes.values():
            for [neighbouring_node_id, edges] in node.adjacency_dict.items():
                for edge in edges.values():
                    if bellman_ford_nodes[node.id].path_weight != INT_MAX and bellman_ford_nodes[node.id].path_weight + edge.weight < bellman_ford_nodes[neighbouring_node_id].path_weight:
                        bellman_ford_nodes[neighbouring_node_id].path_weight = bellman_ford_nodes[node.id].path_weight + edge.weight
                        bellman_ford_nodes[neighbouring_node_id].previous_node = bellman_ford_nodes[node.id]
                        bellman_ford_nodes[neighbouring_node_id].previous_edge = edge
        ...
        for [node1, node2, edge] in graph.edge_list.values():
            if bellman_ford_nodes[node1.id].path_weight != INT_MAX and bellman_ford_nodes[node1.id].path_weight + edge.weight < bellman_ford_nodes[node2.id].path_weight:
